...
def to_hf(state_dict: dict[str, Any], model_args) -> dict[str, Any]:
    to_hf_map = {v: k for k, v in from_hf_map.items()}

    n_heads = model_args.n_heads
    n_kv_heads = (
        model_args.n_kv_heads
        if model_args.n_kv_heads is not None
        else n_heads
    )
    dim = model_args.dim
    ...
    def _sanitize(key: str) -> str:
        # Remove wrapper module prefixes introduced by compile/checkpoint/FSDP
    ...
    for raw_key, value in state_dict.items():
        ...
        if "layers" in key:
            ...
            if abstract_key == "layers.{}.attention.wk.weight":
                ...
                value = _permute(value, n_kv_heads, key_value_dim, dim)
...
def from_hf(hf_state_dict: dict[str, Any], model_args) -> dict[str, Any]:
    n_heads = model_args.n_heads
    n_kv_heads = (
        model_args.n_kv_heads
        if model_args.n_kv_heads is not None
    ...
    for key, value in hf_state_dict.items():
        if "layers" in key:
            abstract_key = re.sub(r"(\d+)", "{}", key, count=1)
            layer_num = re.search(r"\d+", key).group(0)
            new_key = from_hf_map[abstract_key]

            # We need to permute the weights in wq and wk layer in order to account for the difference between
            # the native Llama and huggingface RoPE implementation.
            if abstract_key == "model.layers.{}.self_attn.q_proj.weight":
                value = _reverse_permute(value, n_heads)
            if abstract_key == "model.layers.{}.self_attn.k_proj.weight":
                key_value_dim = head_dim * n_kv_heads
                value = _reverse_permute(value, n_kv_heads, key_value_dim, dim)

            if new_key is None:
                continue
            new_key = new_key.format(layer_num)
        else:
            new_key = from_hf_map[key]
