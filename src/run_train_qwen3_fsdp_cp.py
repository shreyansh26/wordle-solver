...
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "torchtitan")))

# DeviceMesh for (dp x cp)
try:
    # Prefer the stable import used by TorchTitan
...
# try:
#     # TorchTitan CP helper (optional). There is no train_context in this module.
from torchtitan.distributed.utils import create_context_parallel_ctx  # type: ignore
# except Exception:
#     create_context_parallel_ctx = None  # type: ignore
...
def evaluation(
    ...
    for step, batch in enumerate(eval_dataloader):
        ...
        # Optional Context Parallel region (if available)
        if cp_size > 1 and create_context_parallel_ctx is not None:
            cp_buffers = [inputs["input_ids"], inputs["labels"]]
            cp_seq_dims = [1, 1]
...
def build_device_mesh(world_size: int, cp_size: int) -> DeviceMesh:
    assert world_size % cp_size == 0, "world_size must be divisible by cp_size"
    dp = world_size // cp_size
    # Global logical ranks laid out as (dp, cp)
    mesh_2d = torch.arange(world_size, device="cpu").view(dp, cp_size)
    device_mesh = DeviceMesh("cuda", mesh_2d, mesh_dim_names=("dp", "cp"))
    return device_mesh

def pad_for_cp(inputs, pad_token_id, cp_size):
    multiple = 2 * cp_size  # CP "allgather" needs seq % (2*cp) == 0
    ...
    # pad input_ids
    pad_ids = inputs["input_ids"].new_full((B, r), pad_token_id)
    inputs["input_ids"] = torch.cat([inputs["input_ids"], pad_ids], dim=1)

    # pad labels with IGNORE_INDEX
    pad_lbl = inputs["labels"].new_full((B, r), IGNORE_INDEX)
    inputs["labels"] = torch.cat([inputs["labels"], pad_lbl], dim=1)

    # pad attention_mask if present
    if "attention_mask" in inputs:
        pad_mask = inputs["attention_mask"].new_zeros((B, r))
        inputs["attention_mask"] = torch.cat([inputs["attention_mask"], pad_mask], dim=1)

    # pad position_ids if present (values won’t matter for masked pads)
    if "position_ids" in inputs:
        last = inputs["position_ids"][:, -1:]
        pad_pos = torch.arange(1, r + 1, device=inputs["position_ids"].device).unsqueeze(0).expand(B, -1) + last
        inputs["position_ids"] = torch.cat([inputs["position_ids"], pad_pos], dim=1)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--cp-size", type=int, default=1, help="Context parallel degree (>=1). 1 disables CP")
    parser.add_argument("--cp-rotate-method", type=str, default="allgather", help="Context parallel rotate method for position encodings")
    ...
    # Build (dp x cp) mesh; pass DP sub-mesh to FSDP2
    cp_size = max(1, int(args.cp_size))
    world_mesh = build_device_mesh(world_size, cp_size)
    ...
    notes = "qwen3_4b_fsdp2_cp_packing=ffd_flash_attn_fsdp2_torch_compile_dcp_kimi_k2_v2_sft"
    ...
    if local_rank == 0:
        ...
        print(f"WORLD_SIZE: {world_size}, CP_SIZE: {cp_size}, DP_SIZE: {world_size // cp_size}")
    ...
    if local_rank == 0:
        run = wandb.init(
            project="combined_sft_qwen3_4b_fsdp2_cp_v1",
            name=run_id,
            config={
    ...
    for epoch in range(0, epochs):
        ...
        while True:
            ...
            for acc_step in range(gradient_accumulation_steps):
                ...
                # Ensure CP precondition
                inputs = pad_for_cp(inputs, tokenizer.pad_token_id, cp_size)
                ...
                if cp_size > 1 and create_context_parallel_ctx is not None:
                    ...
                    if "attention_mask" in inputs:
                        cp_buffers.append(inputs["attention_mask"])
                        cp_seq_dims.append(1)

                    if "position_ids" in inputs:
                        cp_buffers.append(inputs["position_ids"])
                        cp_seq_dims.append(1)

                    cp_no_restore = {buf for buf in cp_buffers if buf is not None}
                    cp_ctx = create_context_parallel_ctx(
                        cp_mesh=cp_mesh,
                        cp_buffers=cp_buffers,
                        cp_seq_dims=cp_seq_dims,
                        cp_no_restore_buffers=cp_no_restore,
                        cp_rotate_method=args.cp_rotate_method,
                    )
                    ctx_mgr = cp_ctx
                else:
                    ctx_mgr = nullcontext()
